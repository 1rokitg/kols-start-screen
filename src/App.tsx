import { useState, useEffect } from "react";
import StartPage from "./components/StartPage";
import HaveBusiness from "./components/HaveBusiness";
import WantBusiness from "./components/WantBusiness";
import Background from "./components/Background";

type Screen = "start" | "have-business" | "want-business";

export default function App() {
  const [screen, setScreen] = useState<Screen>("start");
  useEffect(() => {
    window.scrollTo({ top: 0, behavior: "instant" as ScrollBehavior });
  }, [screen]);

  const navigate = (target: Screen) => {
    setScreen(target);
  };

  return (
    <div className="relative min-h-screen w-full overflow-hidden">
      <Background />
      <header className="bg-background border-stroke sticky top-0 z-50 flex h-14 w-full items-center border-b px-6 sm:fixed sm:inset-x-0">
        <div className="mx-auto flex w-full max-w-[1200px] items-center">
          <a aria-label="Whop home" href="/" className="flex items-center">
            <svg
              className="text-gray-12 h-[22px] w-auto"
              fill="none"
              height="26"
              viewBox="0 0 119 26"
              width="119"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clip-path="url(#clip0_877_81966)">
                <path
                  d="M49.8827 20.3443L45.5576 2.99365H49.4193L52.3233 15.0096H52.3748L56.5557 2.99365H59.8716L64.0525 15.0096H64.104L67.0286 2.99365H70.8696L66.5446 20.3443H62.6108L58.2342 8.74266H58.1827L53.7856 20.3443H49.8827Z"
                  fill="currentcolor"
                ></path>
                <path
                  d="M72.0435 2.99365H75.6786V8.74266H75.7301C76.0493 8.15222 76.6878 7.67572 77.4292 7.35461C78.1192 7.05421 79.1181 6.86776 80.0037 6.86776C80.982 6.86776 82.3207 7.11636 83.2784 7.61357C84.2464 8.14186 84.9775 8.84624 85.4615 9.73708C85.9867 10.659 86.2441 11.6948 86.2441 12.855V20.3443H82.609V13.3419C82.609 12.3164 82.3104 11.5291 81.7028 10.9697C81.0952 10.4104 80.2714 10.1307 79.221 10.1307C78.1707 10.1307 77.3365 10.4311 76.6672 11.0215C76.0081 11.612 75.6889 12.3889 75.6889 13.3419V20.3443H72.0538V2.99365H72.0435Z"
                  fill="currentcolor"
                ></path>
                <path
                  d="M87.521 13.7976C87.521 12.7617 87.7166 11.8087 88.108 10.949C88.4993 10.0892 89.0348 9.36409 89.7144 8.76329C90.3941 8.1625 91.1767 7.69636 92.0623 7.36489C92.9479 7.03341 93.885 6.86768 94.8633 6.86768C95.8416 6.86768 96.8302 7.03341 97.7261 7.37525C98.622 7.71708 99.3944 8.18321 100.064 8.78401C100.723 9.38481 101.248 10.1099 101.639 10.9593C102.02 11.8087 102.216 12.7513 102.216 13.7872C102.216 14.8231 102.02 15.7968 101.629 16.6462C101.238 17.4956 100.702 18.2207 100.033 18.8215C99.3635 19.4223 98.5808 19.8884 97.6849 20.2199C96.789 20.5513 95.8519 20.7171 94.8736 20.7171C93.8953 20.7171 92.9273 20.5513 92.0314 20.2095C91.1355 19.8677 90.3632 19.4015 89.6938 18.7904C89.0348 18.1792 88.4993 17.4541 88.108 16.6047C87.7166 15.745 87.521 14.8127 87.521 13.7976ZM98.5808 13.7976C98.5808 12.6788 98.241 11.7776 97.5614 11.1043C96.8817 10.431 95.9858 10.0892 94.8736 10.0892C94.3278 10.0892 93.8336 10.1824 93.3702 10.3585C92.9067 10.545 92.5154 10.7936 92.1859 11.1147C91.8564 11.4358 91.6092 11.8294 91.4239 12.2852C91.2385 12.7513 91.1561 13.2486 91.1561 13.7976C91.1561 14.3466 91.2488 14.8438 91.4239 15.2996C91.6092 15.7553 91.8667 16.149 92.1962 16.4701C92.536 16.8015 92.9273 17.0501 93.3804 17.2366C93.8336 17.4231 94.3278 17.5059 94.8736 17.5059C95.4194 17.5059 95.9137 17.4127 96.3771 17.2366C96.8405 17.0501 97.2318 16.8015 97.5614 16.4804C97.8909 16.1593 98.1483 15.7657 98.3234 15.3099C98.4985 14.8438 98.5808 14.3362 98.5808 13.7976Z"
                  fill="currentcolor"
                ></path>
                <path
                  d="M103.544 6.85742H107.035V8.74268H107.087C107.313 8.24547 108.271 7.41679 109.033 7.19926C109.785 6.98173 110.547 6.86778 111.319 6.86778C112.298 6.86778 113.204 7.05423 114.028 7.41679C114.851 7.77934 115.552 8.27655 116.159 8.89806C116.757 9.52993 117.22 10.255 117.56 11.0941C117.9 11.9331 118.064 12.8343 118.064 13.7873C118.064 14.7403 117.9 15.6933 117.56 16.5323C117.22 17.3714 116.757 18.1068 116.159 18.718C115.562 19.3395 114.851 19.8264 114.017 20.1785C113.193 20.5307 112.287 20.7068 111.33 20.7068C111.021 20.7068 110.671 20.6758 110.289 20.624C109.898 20.5618 109.527 20.4789 109.157 20.365C108.786 20.2511 108.436 20.106 108.096 19.9299C107.756 19.7538 107.478 19.557 107.252 19.3188H107.2V25.8965H103.565L103.544 6.85742ZM114.409 13.7977C114.409 12.6789 114.069 11.7777 113.399 11.1148C112.73 10.4518 111.834 10.11 110.722 10.11C110.331 10.11 109.919 10.1722 109.507 10.3068C109.085 10.4415 108.704 10.6279 108.364 10.8765C108.024 11.1252 107.736 11.4256 107.519 11.7674C107.303 12.1092 107.19 12.5132 107.19 12.9586V14.616C107.19 15.1961 107.355 15.6933 107.694 16.1283C108.024 16.553 108.456 16.8845 108.961 17.1124C109.486 17.3714 110.084 17.506 110.753 17.506C111.855 17.506 112.74 17.1642 113.41 16.4805C114.069 15.7969 114.409 14.8957 114.409 13.7977Z"
                  fill="currentcolor"
                ></path>
                <g clipPath="url(#clip1_877_81966)">
                  <path
                    d="M6.29654 0.0205078C3.70387 0.0205078 1.92397 1.15863 0.565903 2.45194C0.565903 2.45194 0.0206171 2.96927 0.0309055 2.98996L5.71011 8.70125L11.3893 2.98996C10.309 1.50006 8.28221 0.0205078 6.29654 0.0205078Z"
                    fill="#FA4616"
                  ></path>
                  <path
                    d="M20.3194 0.0205078C17.7267 0.0205078 15.9468 1.16897 14.5888 2.45194C14.5888 2.45194 14.0949 2.95892 14.0641 2.98996L7.04736 10.0463L12.7163 15.7472L25.4019 2.98996C24.3216 1.50006 22.2948 0.0205078 20.3194 0.0205078Z"
                    fill="#FA4616"
                  ></path>
                  <path
                    d="M34.3738 0.0205078C31.7811 0.0205078 30.0012 1.16897 28.6431 2.45194C28.6431 2.45194 28.1287 2.95893 28.1081 2.98997L14.0645 17.1026L15.546 18.5925C17.8403 20.8998 21.6059 20.8998 23.9105 18.5925L39.4357 2.98997C38.3657 1.50006 36.3594 0.0205078 34.3738 0.0205078Z"
                    fill="#FA4616"
                  ></path>
                </g>
              </g>
              <defs>
                <clipPath id="clip0_877_81966">
                  <rect fill="white" height="26" width="118.116"></rect>
                </clipPath>
                <clipPath id="clip1_877_81966">
                  <rect fill="white" height="20.3648" width="39.5021"></rect>
                </clipPath>
              </defs>
            </svg>
          </a>
          <div className="ml-auto flex items-center gap-2 max-sm:gap-0">
            <a
              href="https://network.whop.com"
              target="_blank"
              rel="noopener noreferrer"
              role="button"
              tabIndex={0}
              data-accent-color="gray"
              className="fui-reset fui-BaseButton fui-Button max-sm:px-2 fui-r-size-2 fui-variant-ghost"
            >
              For enterprise
            </a>
            <a
              href="https://docs.whop.com"
              target="_blank"
              rel="noopener noreferrer"
              role="button"
              tabIndex={0}
              data-accent-color="gray"
              className="fui-reset fui-BaseButton fui-Button max-sm:px-2 fui-r-size-2 fui-variant-ghost"
            >
              API
            </a>
            <button
              type="button"
              tabIndex={0}
              data-accent-color="gray"
              className="fui-reset fui-BaseButton fui-Button max-sm:px-2 fui-r-size-2 fui-variant-ghost"
            >
              Sign in
            </button>
          </div>
        </div>
      </header>
      <div className="relative z-10">
        {screen === "start" && (
          <StartPage
            onHaveBusiness={() => navigate("have-business")}
            onWantBusiness={() => navigate("want-business")}
          />
        )}
        {screen === "have-business" && (
          <HaveBusiness onBack={() => navigate("start")} />
        )}
        {screen === "want-business" && (
          <WantBusiness onBack={() => navigate("start")} />
        )}
      </div>
    </div>
  );
}
